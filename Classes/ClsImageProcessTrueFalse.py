...
class ClsImageProcessTrueFalse(ClsImageProcess):
    ...
    def process(self):
        ...
        if self.sTiltLeftForearm > 110 and self.sTiltRightForearm < -110:
            if self.blCrossArms:
                self.strPose = 'Batsu'
            elif self.sTiltLeftUpperArm < -120 and self.sTiltRightUpperArm > 120:
                self.strPose = 'Maru'

        self.showParams()
        self.imProcessed = self.imSensor

        if self.strPose == 'Maru':
            return True
        else:
            return False
...
if __name__ == "__main__":
    ...
    cLogger = ClsLogger("test-ClsImageProcessTrueFalse")
    ...
    cProc = ClsImageProcessTrueFalse(cLogger, cImageSensor, cWindow)
